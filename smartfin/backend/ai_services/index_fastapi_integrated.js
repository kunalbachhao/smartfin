...
// ================= ML SMS CATEGORY ROUTE =================
const ML_API_URL = process.env.ML_API_URL || "http://127.0.0.1:8000";

app.post("/predict-sms-category", async (req, res) => {
  try {
    let { sms } = req.body;
    ...
    if (!sms || typeof sms !== "string" || !sms.trim()) {
      return res.status(400).json({
        success: false,
        message: "sms is required and must be a non-empty string"
      });
    }
    ...
    return res.json({
      success: true,
      message: "SMS category predicted successfully",
      data: response.data
    });

  } catch (error) {
    console.error("SMS category prediction error:", error.response?.data || error.message);

    return res.status(error.response?.status || 500).json({
      success: false,
      message: "Failed to predict SMS category",
